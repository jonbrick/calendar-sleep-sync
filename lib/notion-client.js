...
class NotionClient {
  ...
  async createSleepRecord(sleepData) {
    try {
      ...
      console.log(`✅ Created sleep record for ${sleepData.day}`);
      return response;
    } catch (error) {
  ...
  transformSleepToNotion(sleep) {
    ...
    const wakeTimeStr = wakeTime.toLocaleString("en-US", {
      hour: "2-digit",
      minute: "2-digit",
      hour12: true,
      month: "short",
    ...
    return {
      ...
      "Sleep Summary": {
        rich_text: [
          {
            text: {
              content: `${bedtimeStr} → ${wakeTimeStr} (${
                sleep.sleepDurationHours
              }hrs) - Score: ${sleep.sleepScore || "N/A"}`,
            },
          },
        ],
      },
  ...
  transformNotionToSleep(notionPages) {
    return notionPages.map((page) => {
      ...
      return {
        ...
        summary: props["Sleep Summary"]?.rich_text?.[0]?.plain_text,
      };
    });
  }
