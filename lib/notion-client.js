...
class NotionClient {
  ...
  async createSleepRecord(sleepData) {
    try {
      ...
      console.log(
        `✅ Created sleep record: ${properties["Night of"].title[0].text.content}`
      );
      return response;
    } catch (error) {
  ...
  transformSleepToNotion(sleep) {
    ...
    // Calculate "Night of Date" based on Oura date and bedtime
    const nightOfDate = new Date(sleep.day + "T00:00:00"); // Start with Oura's date
    const bedtimeHour = bedtime.getHours();

    // Default: Night of Date = Oura Date - 1
    // Exception: If bedtime is between 12pm-5pm (noon to 5pm), it's a nap, use Oura Date
    if (bedtimeHour < 12 || bedtimeHour >= 17) {
      // Before noon OR after 5pm: belongs to previous day
      nightOfDate.setDate(nightOfDate.getDate() - 1);
    }
    // Between 12pm-5pm: it's a nap, keep Oura's date
